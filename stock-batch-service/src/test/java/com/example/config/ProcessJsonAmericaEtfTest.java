...
@SpringBootTest
@ActiveProfiles("test")
public class ProcessJsonAmericaEtfTest {

    @Autowired
    private JobRepository jobRepository;

    @Autowired
    private Step processJsonAmericaEtfStep;

    @Test
    void processJsonAmericaStockStep_성공_테스트() throws Exception {
        // given
        String filePath = new ClassPathResource("america-etf-nasdaq.json").getFile().getAbsolutePath();
        ...
        // ✅ 중요: JobExecution의 ExecutionContext에 파일 경로 설정
        jobExecution.getExecutionContext().putString("jsonFilePath", filePath);

        // StepExecution 생성
        StepExecution stepExecution = new StepExecution("processJsonAmericaEtfStep", jobExecution);
        ...
        // when
        processJsonAmericaEtfStep.execute(stepExecution);
